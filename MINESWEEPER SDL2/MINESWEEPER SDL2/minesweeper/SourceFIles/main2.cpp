...
int main(int argc, char* argv[])
{
	...
	SDL_Surface * loseScreen = SDL_CreateRGBSurface(0, screen->w, screen->h, 32, 0, 0, 0, 0);
	...
	TTF_Font* font = TTF_OpenFont("C:/Users/messi/source/repos/MINESWEEPER SDL2/Font/peepo/Peepo.ttf", 24);
	if (font == nullptr)
	{
		std::cout << "unable to load" << std::endl;
	}
	SDL_Color text_color = { 255, 0, 0 };
	SDL_Surface* lose_text_surface = TTF_RenderText_Solid(font, "You Lose", text_color);
	SDL_Texture* lose_text_texture = SDL_CreateTextureFromSurface(renderer, lose_text_surface);

	SDL_Rect lose_text_rect;
	lose_text_rect.x = 200; // X-coordinate
	lose_text_rect.y = 240; // Y-coordinate
	lose_text_rect.w = lose_text_surface->w; // Width of the text
	lose_text_rect.h = lose_text_surface->h; // Height of the text
	...
	while (running)
	{
		...
		if (state == LOSE)
		{
			...
			SDL_FillRect(loseScreen, NULL, SDL_MapRGBA(loseScreen->format, color.r, color.g, color.b, color.a));
			SDL_BlitSurface(loseScreen, NULL, screen, NULL);
			SDL_BlitSurface(lose_text_surface, NULL, screen, &lose_text_rect);
